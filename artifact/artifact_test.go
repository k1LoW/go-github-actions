...
func TestUpload(t *testing.T) {
	...
	id, err := Upload(context.TODO(), "TestUpload", "artifact/testdata/test.txt", strings.NewReader("hello artifact 3\n"))
	if err != nil {
		t.Fatal(err)
	}
	if !useLegacy() && id == 0 {
...
func TestUploadLargeContent(t *testing.T) {
	...
	if _, err := Upload(ctx, name, "artifact/testdata/large.txt", strings.NewReader(s)); err != nil {
		t.Error(err)
	}
}
...
func TestUploadFiles(t *testing.T) {
	...
	id, err := UploadFiles(context.TODO(), "TestUploadFiles", files)
	if err != nil {
		t.Fatal(err)
	}
	if !useLegacy() && id == 0 {
...
func TestUploadWithAttestation(t *testing.T) {
	...
	if _, err := Upload(context.TODO(), "TestUploadWithAttestation", "artifact/testdata/attested.txt", strings.NewReader("hello attestation\n"), opt); err != nil {
		t.Error(err)
	}
}
